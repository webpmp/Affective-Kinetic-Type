  ...
  return (
    <div className={isStacked 
      ...
      <div className={containerClasses}>
        ...
        {isStacked && (
          <section className="bg-white border border-slate-200 shadow-sm rounded-xl p-4 min-w-[220px] max-w-[220px] shrink-0 h-full flex flex-col justify-between">
              ...
              <div className="w-full flex items-center justify-between border-b border-slate-100 pb-1.5 mb-2 shrink-0 select-none h-8">
                ...
                {aiProvider === 'lm-studio' && onOpenLMStudioSettings && (
                  <button
                    onClick={onOpenLMStudioSettings}
                    title="Configure LM Studio"
                    className="p-1 rounded-md text-indigo-600 hover:text-indigo-800 hover:bg-slate-100 transition-colors"
                  >
                    <Settings className="w-3.5 h-3.5" />
                  </button>
                )}
              </div>
              <p className="text-[10px] text-slate-500 leading-normal mt-1 flex-1 overflow-y-auto custom-scrollbar">
                {aiProvider === 'offline'
            ...
            <select
              value={aiProvider}
              onChange={(e) => onAIProviderChange(e.target.value as AIProvider)}
              className="w-full mt-4 px-3 py-2 text-xs font-semibold text-slate-700 bg-white hover:bg-slate-50 border border-slate-200 rounded-lg cursor-pointer focus:ring-2 focus:ring-indigo-500/30 focus:outline-none transition-colors appearance-auto"
            >
              {AI_PROVIDER_GROUPS.map(group => (
                <optgroup key={group} label={group}>
                  {AI_PROVIDERS.filter(p => p.group === group).map(p => (
          ...
          {renderSectionContent('typography', (
            <>
              ...
              <div className="space-y-4">
                <label className="text-sm font-medium text-slate-600 block">Typography Library</label>
                <div className="max-h-64 overflow-y-auto pr-1 space-y-4 custom-scrollbar">
                  {Array.from(new Set(FONTS.map(f => f.category))).map(category => (
                      ...
                      <div className="grid grid-cols-1 gap-1">
                        {FONTS.filter(f => f.category === category).map(font => {
                          ...
                          return (
                            <label 
                              key={font.name}
                              className={`flex items-center justify-between p-2 rounded cursor-pointer transition-colors ${isEnabled ? 'bg-indigo-50 border border-indigo-200' : 'hover:bg-slate-50 border border-transparent'}`}
                            >
                              <div className="flex items-center gap-2">
                                <input
                                  type="checkbox"
                                  ...
                                  onChange={(e) => {
                                    if (e.target.checked) {
                                      onEnabledFontsChange([...enabledFonts, font.name]);
                                    } else {
                                      onEnabledFontsChange(enabledFonts.filter(f => f !== font.name));
                                    }
                                  }}
                                  className="rounded text-indigo-600 w-4 h-4"
                                />
                                <span className="text-sm text-slate-700" style={{ fontFamily: font.value }}>
                                  {font.name}
                                </span>
                              </div>
                            </label>
