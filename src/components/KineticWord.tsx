...
export function KineticWord({ 
  ...
  const SAFE_PALETTE = ['#FFFFFF', '#000000', '#00FFFF', '#FFFACD', '#E0FFFF'];
  // We approximate the required contrast text size based on scale and weight
  const isLargeTextForRatio = scale >= 1.2 || fontWeight >= 700;
  const requiredRatio = getRequiredContrast(wcagLevel, isLargeTextForRatio);
  
  let bestColor = emotionColor;
  const ratios = SAFE_PALETTE.map(c => ({ 
    color: c, 
    ratio: getContrastRatio(c, backgroundColor) 
  }));
  
  // Sort descending by ratio
  ratios.sort((a, b) => b.ratio - a.ratio);
  
  if (ratios[0].ratio >= requiredRatio) {
    bestColor = ratios[0].color;
  } else {
    // Fallback: simple inversion of background color based on luminance
    bestColor = isBgLight ? '#000000' : '#FFFFFF';
  }

  emotionColor = bestColor;
